...
- GitHub Release 版：[CodexProxySwitcher-win-x64-v0.1.5.zip](https://github.com/hloolx/codex-proxy-switcher-win/releases/latest)
...
如果 Windows 拒绝直接启动 `C:\Program Files\WindowsApps\...\Codex.exe`，启动器会自动改用 Codex 的 AppUserModelID 进行系统应用激活。VPN 模式下，这个兜底路径会短暂设置当前用户级代理环境变量，激活 Codex 后立即恢复原值，用来绕过 WindowsApps 的目录权限限制。
