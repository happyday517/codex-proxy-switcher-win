...
- GitHub Release 版：[CodexProxySwitcher-win-x64-v0.1.2.zip](https://github.com/hloolx/codex-proxy-switcher-win/releases/latest)
...
本仓库的启动器源码是独立实现，没有复制其他开源项目代码。README 中提到的类似项目只用于说明定位差异，不是代码来源。

源码中如果出现旧名 `CodexLauncher`，仅用于迁移早期版本保存到 `%APPDATA%\CodexLauncher\settings.json` 的本地端口配置。新版本配置目录是 `%APPDATA%\CodexProxySwitcher\settings.json`。

## 当前优化点

- 项目名、程序集名和命名空间已经统一为 `CodexProxySwitcher`。
- 构建脚本输出统一命名为 `Codex-Proxy-Switcher.exe`。
- 保留旧配置迁移，避免升级后需要重新输入端口。
- Release 包使用自包含 win-x64 版本，普通用户不需要手动安装 .NET Runtime。
